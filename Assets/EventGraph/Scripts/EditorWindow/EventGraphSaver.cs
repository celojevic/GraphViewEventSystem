...
namespace EventGraph
{
    public static class EventGraphSaver
    {
        ...
        /// <summary>
        /// EventGraph namespace including the '.'
        /// </summary>
        private const string NAMESPACE = "EventGraph.";


        public static string AppendNamespace(string input)
        {
        ...
        private static void ReconstructGraph(EventGraphView graphView, EventGraphData graphData)
        {
            ...
            for (int i = 0; i < graphData.nodeJsons.Count; i++)
            {
                ...
                Type dataType = Type.GetType(AppendNamespace(nodeData.nodeDataType));
                var data = JsonUtility.FromJson(graphData.nodeJsons[i], dataType);

                // create node from loaded save data
                Type nodeType = Type.GetType(AppendNamespace(nodeData.nodeType));
